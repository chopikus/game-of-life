...
class Hashlife {
   ...
   */
   NodePtr _centre(NodePtr m);

   /* Computes next step for node E*/
   /* | a b c |
      | d E f |
      | g h i | */
   NodePtr _life_3x3(NodePtr a, NodePtr b,NodePtr c,
   ...
   void _append_alive_cells(NodePtr node, std::vector<Cell>& output,
                           uint8_t level,
                           int64_t x, int64_t y,
